...
def plot_Pareto_Fronts(optimization_mode, opt_results, optimal_solutions_df,sys_config,optimization_specs):
    ...
    if optimization_mode == True:
        ...
        for fuel in fuel_types:
            for ems in ev_ems_types:
                for idx, reno in enumerate(renovation_types):
                    ...
                    try:
                        x_results[key] = [opt_results[0][i] for i in range(len(fuel_type)) if fuel_type[i] == fuel and ev_ems_type[i] == ems and renovation_type[i] == reno]
                        y_results[key] = [opt_results[1][i] for i in range(len(fuel_type)) if fuel_type[i] == fuel and ev_ems_type[i] == ems and renovation_type[i] == reno]
                    except:
                        x_results[key] = [opt_results["Annuity Cost"][i] for i in range(len(fuel_type)) if fuel_type[i] == fuel and ev_ems_type[i] == ems and renovation_type[i] == reno]
                        y_results[key] = [opt_results["CO2 emissions"][i] for i in range(len(fuel_type)) if fuel_type[i] == fuel and ev_ems_type[i] == ems and renovation_type[i] == reno]
            ...
            for ems in ev_ems_types:
                for idx, reno in enumerate(renovation_types):
                    ...
                    if len(x_results[key])>0:
                        ...
                        plt.plot(x_results[key], y_results[key], markersize= 15, markeredgewidth=3, linestyle='', marker= marker_fuel, markeredgecolor= marker_ems, color=marker_reno, label=f"Gas:[{fuel}]  Car:[{ems}]  Renovation:[{reno}]")
        ...
        folder_name = os.path.join(path+"\Output\Optimizations\Illustrations", dt.date.today().strftime('%Y-%m-%d'))
        try: 
            #create the folder first if it does not exist
            os.makedirs(folder_name)
            
            plt.savefig(folder_name + '\\ParetoFronts{}_{}_Gen{}_Pop{}_{}.eps'.format(sys_config, optimization_specs["Algorithm"],optimization_specs["Generations"],optimization_specs["Population Size"],dt.date.today()), dpi=300, format='eps')
            plt.savefig(folder_name + '\\ParetoFronts{}_{}_Gen{}_Pop{}_{}.png'.format(sys_config, optimization_specs["Algorithm"],optimization_specs["Generations"],optimization_specs["Population Size"],dt.date.today()), dpi=300, format='png')
        except:
            plt.savefig(folder_name + '\\ParetoFronts{}_{}_Gen{}_Pop{}_{}.eps'.format(sys_config, optimization_specs["Algorithm"],optimization_specs["Generations"],optimization_specs["Population Size"],dt.date.today()), dpi=300, format='eps')
            plt.savefig(folder_name + '\\ParetoFronts{}_{}_Gen{}_Pop{}_{}.png'.format(sys_config, optimization_specs["Algorithm"],optimization_specs["Generations"],optimization_specs["Population Size"],dt.date.today()), dpi=300, format='png')
...
def sensivity_analysis(lower_boundary, Sensitivity_step, upper_boundary, sensitivity_variable, specs_eco,optimization_mode,optimization_specs,combined_heat_and_power,max_pv_power,temp_data,energy_system_simulation,own_design,n):
    ...
    Sensitivity_results= pd.DataFrame(columns= ["optimization results","CO2 emissions", "Annuity Cost", f"{sensitivity_variable}"])
    ...
    for price in np.arange(lower_boundary,upper_boundary,Sensitivity_step):
        ...
        Sensitivity_results_electricity_annuity.append(optimization_results.F.T[0])
...
def plotting_Sensitivity(ref, Sensitivity_results, Sensitivity_test):
    ...
    df= pd.DataFrame(columns=["CO2 emissions","Annuity Cost","{} price".format(Sensitivity_test)])
    # df= pd.DataFrame()
    ...
    for i in Sensitivity_results.index:
        ...
        if isinstance(ii, str):
            emissions_float = [float(x) for x in ii.strip("[]").split()]  #changes list of string to list of floating numbers
            z = [1]*len(emissions_float)
            result = [x * Sensitivity_results['{} price'.format(Sensitivity_test)][i] for x in z]
            df['{} price'.format(Sensitivity_test)][i] = result
            df['{} price'.format(Sensitivity_test)][i] = np.array(df['{} price'.format(Sensitivity_test)][i])
    ...
    df['{} price'.format(Sensitivity_test)] = df['{} price'.format(Sensitivity_test)].apply(parse_array)
    ...
    ax.set_xlabel('Annuity Cost [10$^{3}$ €$\cdot a^{-1}$]', fontsize=20, labelpad= 40)
   ...
   # plt.title('Change of CO2 emmisions and Annuity prices according to change in {} prices'.format(Sensitivity_test), fontsize=30)
    ...
    try: 
        ...
        os.makedirs(folder_name)
        #save
        plt.savefig( folder_name +'\\Sensitivity3D_{}.eps'.format(Sensitivity_test), dpi=300, format='eps')
        plt.savefig( folder_name +'\\Sensitivity3D_{}.png'.format(Sensitivity_test), dpi=300, format='png')
    except:
        plt.savefig( folder_name +'\\Sensitivity3D_{}.eps'.format(Sensitivity_test), dpi=300, format='eps')
        plt.savefig( folder_name +'\\Sensitivity3D_{}.png'.format(Sensitivity_test), dpi=300, format='png')
...
def Sensitivity_data_prep(Sensitivity_results,test):
    ...
    for i in Sensitivity_results.index:
        ...
        Sensitivity_results[f"{test} price"][i] = float(Sensitivity_results[f"{test} price"][i])
        Sensitivity_results["hyper volume"][i] = float(Sensitivity_results["hyper volume"][i])
        Sensitivity_results["spacing"][i] = float(Sensitivity_results["spacing"][i])
...
def plot_PCP(ResultPath, ParetoFrontPath_Opt_Sol, ParetoFrontPath):
    ...
    ParetoFronts_Opt_Solutions.iloc[:,7] = np.ceil(ParetoFronts_Opt_Solutions.iloc[:,7] )
    ParetoFronts_Opt_Solutions.iloc[:,8] = np.ceil(ParetoFronts_Opt_Solutions.iloc[:,8] )
    ParetoFronts_Opt_Solutions.iloc[:,9] = np.floor(ParetoFronts_Opt_Solutions.iloc[:,9] )
    ...
    final_results_tradeoff = np.array([ParetoFronts_Results.iloc[:,1], ParetoFronts_Results.iloc[:,2]]) 
    ...
    final_results_tradeoff, pseudo_weights_2023_sys_a_tradeoff = PseudoWeights(weights).do(final_results_tradeoff, return_pseudo_weights=True)
    ...
    pcp_plot_a_2023.add(final_results_pcp[1], alpha=0.9, color="darkred", linewidth=2.5, label="min. CO$_{2}$")
    pcp_plot_a_2023.add(final_results_pcp[0], alpha=0.9, color='olivedrab', linewidth=2.5, label="min. C$_{a}$")
    pcp_plot_a_2023.add(final_results_pcp[final_results_tradeoff], linestyle='-.', alpha=0.5, color='darkcyan',linewidth=3, label="eq. w. Trade-Off")



    pcp_plot_a_2023.show()

    plt.savefig(result_path + '\\PCP_Final_Results_{}.eps'.format(dt.date.today()), dpi=300, format='eps')
    plt.savefig(result_path + '\\PCP_Final_Results_{}.png'.format(dt.date.today()), dpi=300, format='png')