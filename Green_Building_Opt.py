...
from joblib import Parallel, delayed   #this library is used to paralize running fucntions on all CPU cores
...
from plotting_functions import sensivity_analysis #function performs Sensitivity analysis
...
optimization_mode = False                                                       # If True, the programm carries out a multi-objective optimization. If False only a timerseries simulation for the design specified by the Decision Variables below in the row below and the component dicts is carried out.
combined_heat_and_power = False                                                # If True, the energy system including a fuel cell chp component is simulated
output_results_as_table = False                                                 # If true, give output results also as table
Sensitivity_analysis = False                                                   # If True there is a Sensitivity analysis to be performed
plot_results = False
...
df_owndesign_parameters = pd.read_excel(filepath_input_parameters, sheet_name="Own Design", index_col=0, usecols="A:B")

if combined_heat_and_power == False:                                            # System Design without CHP
    df_owndesign_parameters_without_CHP = df_owndesign_parameters.iloc[2: 11]
    own_design = df_owndesign_parameters_without_CHP.to_numpy(dtype="float").reshape((1,9))
    ...
    own_design = df_owndesign_parameters_with_CHP.Values.to_numpy(dtype="float").reshape((1,10))
...
df_general_parameters = pd.read_excel(filepath_input_parameters, sheet_name="General Parameters", index_col=0, usecols="A:B")
...
scaling_factor_electric_energy_demand = df_general_parameters["Values"].loc[df_general_parameters.index[7]]
...
df_opti_parameters = pd.read_excel(filepath_input_parameters, sheet_name="Optimization Parameters", index_col=0, usecols="A:B")
...
df_tech_parameters = pd.read_excel(filepath_input_parameters, sheet_name="Technical Parameters", index_col=0, usecols="A:B")
...
df_ecological_parameters = pd.read_excel(filepath_input_parameters, sheet_name=5, index_col=0, usecols="A:B")
...
df_eco_parameters = pd.read_excel(filepath_input_parameters, sheet_name="Economical Parameters", index_col=0, usecols="A:E", keep_default_na=False)

df_eco_parameters_specs = df_eco_parameters.iloc[2:47]
values_capex_hp = df_eco_parameters_specs.loc["CAPEX HP"].values.flatten().tolist()
values_capex_battery = df_eco_parameters_specs.loc["CAPEX Battery"].values.flatten().tolist()
values_capex_ww = df_eco_parameters_specs.loc["CAPEX Water Storage"].values.flatten().tolist()
values_capex_h2boiler = df_eco_parameters_specs.loc["CAPEX H2 Boiler"].values.flatten().tolist()
values_capex_h2chp = df_eco_parameters_specs.loc["CAPEX H2 CHP"].values.flatten().tolist()
df_eco_parameters_specs = df_eco_parameters_specs.drop(index=["CAPEX HP", "CAPEX Battery", "CAPEX Water Storage", "CAPEX H2 Boiler", "CAPEX H2 CHP"], columns=["Values (b)", "Reference Year", "Cost Reduction to 2023 [%]"])
df_eco_parameters_specs.rename(columns = {"Values (a)" : "specs_eco"}, inplace = True)
...
df_renov_parameters_specs = df_renov_parameters_specs.drop(columns=["Values (b)", "Reference Year", "Cost Reduction to 2023 [%]"])
df_renov_parameters_specs.rename(columns = {"Values (a)" : "specs_renovation"}, inplace = True)
...
building_data_filenames_0 = glob.glob(path + "/Input/buildings/HT26 (MA Thesis)/ohne_Sanierung/*.csv")
building_data_filenames_1 = glob.glob(path + "/Input/buildings/HT26 (MA Thesis)/Sanierung_Dach/*.csv")
building_data_filenames_2 = glob.glob(path + "/Input/buildings/HT26 (MA Thesis)/Sanierung_DachFenster/*.csv")
building_data_filenames_3 = glob.glob(path + "/Input/buildings/HT26 (MA Thesis)/Sanierung_DachFensterWände/*.csv")
building_data_filenames_4 = glob.glob(path + "/Input/buildings/HT26 (MA Thesis)/Sanierung_DachFensterWändeKeller/*.csv")
building_data_filenames_5 = glob.glob(path + "/Input/buildings/HT26 (MA Thesis)/Sanierung_vollständig/*.csv")

atmospheric_data_filenames = glob.glob(path + "/Input/atmosphere/*")


# # Data Imports from Sheets
co2_data = pd.read_csv(atmospheric_data_filenames[0], sep=';', decimal=",", encoding= 'unicode_escape') # Emission timeline Data Import

temp_data = pd.read_excel(atmospheric_data_filenames[1]) # Temperature Data Import

solar_roof_tile_data = pd.read_excel(atmospheric_data_filenames[2]) # Cooled Module Temperatures & Heat Flux Temperatures for SRTs from Research Project SolardachpfanneNRW

irrad_data = {"South": 3, "West": 4, "East": 5}
wind_data = {"South": 1, "West": 2, "East": 3}
wind_timeseries = pd.read_excel(atmospheric_data_filenames[6]) # Wind Data Import
for direction in irrad_data.keys():
    # read irradiation timeline for given direction
    irrad_data[direction] = pd.read_csv(atmospheric_data_filenames[irrad_data[direction]], sep=',', decimal=".", encoding= 'unicode_escape', skiprows = 8, skipfooter = 12, engine = "python")
    irrad_data[direction]["Timestamp"] = timestamp
    irrad_data[direction]["POA Global [W/m2]"] = irrad_data[direction]["Beam inplane [W/m2]"] + irrad_data[direction]["Diffuse inplane [W/m2]"]
    ...
    if direction == "South":
        wind_data[direction].loc[~wind_data[direction]["Wind Direction [°]"].between(90,270, inclusive='both'), 'Wind Speed [m/s]'] = 0 # 180 ist Süd, 0 ist Nord, 90 ist Ost und 270 ist West
    elif direction == "West":
        wind_data[direction].loc[~wind_data[direction]["Wind Direction [°]"].between(180,360, inclusive='both'), 'Wind Speed [m/s]'] = 0
    elif direction == "East":
        wind_data[direction].loc[~wind_data[direction]["Wind Direction [°]"].between(0,180, inclusive='both'), 'Wind Speed [m/s]'] = 0
    else:
        pass
...
solar_roof_tile_data["Temperature Difference Heat Flux [°C]"] = solar_roof_tile_data["Tmcooling [°C]"] - solar_roof_tile_data["T heatflux [°C]"]
solar_roof_tile_data.drop(solar_roof_tile_data.index[len(solar_roof_tile_data)-2:], inplace=True) # Deleting last two rows of Data Frame (Sum and Mean of Input data)

# Temperature gain for Heatflux from Solar rooftile Heat - Estimate Coefficients from Regression
regression_coefficients, r_regression = heat_flux_temperature_difference(solar_roof_tile_data, irrad_upper=1100)

# CO2 Emissionfactor resampling
co2_data = signal.resample(co2_data, 8759)  # Resample CO2 Emission Data to 8759 d/a
...
rs_series = fit_cec_sam(celltype=hybrid_solar_rooftiles["type"],
                              ...
                              alpha_sc= hybrid_solar_rooftiles["Temperature_coeff._alpha"]*hybrid_solar_rooftiles["Ik"],
                              beta_voc= hybrid_solar_rooftiles["Temperature_coeff._betha"]*hybrid_solar_rooftiles["UL"],
...
hybrid_solar_rooftiles["Series Resistance Rs"] = rs_series[2] # Asign fitted series resistance
hybrid_solar_rooftiles["solar_cell_area"] = round((hybrid_solar_rooftiles["Umpp"] * hybrid_solar_rooftiles["Impp"]) / (hybrid_solar_rooftiles["eta_el_stc"] / 100 * hybrid_solar_rooftiles["Irradiation STC"]), 4) # estimated cell area
hybrid_solar_rooftiles["p_area"] = hybrid_solar_rooftiles["p_max"] / hybrid_solar_rooftiles["solar_cell_area"]  # max power per area

max_number_of_tiles = int(building["area_roof"] * hybrid_solar_rooftiles["tiles_per_sqm"])
...
def energy_system_simulation(designs): #This works, but normally the system component parameters have to be passed in the function call, and ultimately into the system design problem calls, which is oriented to the pymoo structure. An idea is to pass the component dicts by kwargs**.
    ...
    def threaded_function(j,designs,result_list):
        ...
        pv_power_west=float(design.item(0)),                       # Installed PV Power on west roof[W]
        pv_power_east=float(design.item(1)),                    # Installed PV Power on east roof[W]
        storage_capacity_electric=float(design.item(2)),      # Installed Battery Storage Capacity [kW]
        storage_capacity_thermal=float(design.item(3)),       # Installed Warm Water Storage Volume [L]
        hp_nominal_power=float(design.item(4)),               # Heat Pump nominal Power [kW]
        hp_bivalency_point=float(design.item(5))              # Heat Pump Bivalency Point [°C]
        
        if combined_heat_and_power == True: # If CHP should not be simulated, the installed capacity is set to 0
            chp_specs["p_el_nom_chp"] = float(design.item(6)) # CHP Installed capacity electric [kW]
            fuel = float(design.item(7))
            ev_ems = float(design.item(8))
            renovation_case = float(design.item(9))               # House Renovation Status
        else:
            chp_specs["p_el_nom_chp"] = 0
            fuel = float(design.item(6))
            ev_ems = float(design.item(7))
            renovation_case = float(design.item(8))               # House Renovation Status
        ...
        if ev_ems >= 1:
            ev_df["Dataframe"]= ev_ems_connection_profile(                                          #plug-in state of the vehicle, showing when the car is avaible for charging
                                        ...
                                        arrival_time=ev_df["arrival time"],                         #time returning home from work
                                        departure_time=ev_df["departure time"],                     #time leaving home for work
                                        vacation_departure_time= ev_df["vacation departure time"],  #time leaving home for vacation
                                        vacation_arrival_time= ev_df["vacation arrival time"],      #time returning home from vacation
        ...
        building_demand["Sum Electricity [kWh]"] = building_demand["Sum Electricity [kWh]"] * scaling_factor_electric_energy_demand
        ...
        e_electrical_vehicle_t_min_1 = ev_df["Battery Capacity"] * 0.5               #Electrical vehicle's initial battery capacity
        soc_ev_t_min_1 = e_electrical_vehicle_t_min_1/ ev_df['Battery Capacity']    #initial state of charge according to battery size
        
        building["area_solar_roof_tiles_west"] = hybrid_solar_rooftiles["p_installed_west"] / hybrid_solar_rooftiles["p_area"]
        building["area_solar_roof_tiles_east"] = hybrid_solar_rooftiles["p_installed_east"] / hybrid_solar_rooftiles["p_area"]
        building["area_solar_roof_tiles"] = building["area_solar_roof_tiles_east"] + building["area_solar_roof_tiles_west"] 

        fan_heat_pump["air volume flow"] = fan_heat_pump["v_ratio"] * building["area_solar_roof_tiles"]                             # Calculating air volume flow(solar roof tiles area)
        fan_heat_pump["p_el"] = (fan_heat_pump["p_max"]/fan_heat_pump["air volume flow max"]) * fan_heat_pump["air volume flow"] / 1000    # Interpolating p_el(air volume flow) [kW]
        ...
        # Heat Exchanger
        ww_storage["solar_circle_he_area"] = ww_storage["V_storage"] * ww_storage["solar_circle_he_ratio"]
        ww_storage["warm_water_he_area"] = ww_storage["V_storage"] * ww_storage["warm_water_circle_he_ratio"]

        # Maximum capacity of warm water storage [kWh_th] as in DIN EN ISO 12831 (2017).  Conversions: 1h/3600s
        ww_storage["Q_storage_nom"] = ww_storage["V_storage"] * rho_w * cp_w * (ww_storage["t_upper"] - ww_storage["t_lower"]) *  ww_storage["load_factor_f1"] * (1/3600)
        ...
        q_loss_regression_coefficients, r_regression_qloss = q_loss_specific_din12831(combined_heat_and_power)
        ww_storage["q_storage_loss"] = q_loss_regression(ww_storage["V_storage"], q_loss_regression_coefficients[0][0]) * ((ww_storage["t_upper"] - ww_storage["t_a"])/45)

        ww_storage["t_storage_lower_min"] = abs(ww_storage["t_upper"]-2*ww_storage["t_sensor"]) # estimated minimum Temperature at the bottom of the storage to ensure the Set Temperature at the sensor
        # q_storage_on will be the energy level the thermal storage has to contain after a timestep
        ww_storage["q_storage_on"] = ww_storage["V_storage"] * rho_w * cp_w * (1 - (ww_storage["h_sensor"]/(2*ww_storage["h_storage"]))) * (ww_storage["t_sensor"] - ww_storage["t_lower"])  * (1/3600) *  ww_storage["load_factor_f1"]

        # HP
        heat_pump["Bivalency Point"] = hp_bivalency_point                                              # Bivalency Point
        heat_pump["q_nom"] = hp_nominal_power[0]                                                       # nominal Power of Heat Pump in kW

        # Electrical Storage
        electrical_storage["e_electrical_storage_nom"] = storage_capacity_electric[0]                                                                   # Nominal Storage Capacity of electrical Storage [kWh_el]
        electrical_storage["e_electrical_storage_usable"] = electrical_storage["e_electrical_storage_nom"] * electrical_storage["Discharge Depth max"]  # Usable Storage Capacity of electrical Storage [kWh_el]
        electrical_storage["e_electrical_storage_initial"] = electrical_storage["e_electrical_storage_usable"] * 0.5                                    # Initial Storage Capacity of electrical Storage [kWh_el]
        ...
        soc_t_min_1 = (electrical_storage["e_electrical_storage_initial"]/electrical_storage["e_electrical_storage_usable"]) * 100
        ...
        for i in building_demand.index[0:8759]:
            ...
            system_ops = energy_system_operating_strategy(irradiation = irrad_data["West"]["POA Global [W/m2]"][i] + irrad_data["East"]["POA Global [W/m2]"][i],
                                                          ...
                                                          heat_pump = heat_pump,
                                                          combined_heat_and_power = combined_heat_and_power # Decision Variable for CHP simulation (System Configuration B)
                                                          )
            ...
            residual_load_electric = solar_thermal_energy.p_solar_roof_tile_electrical_ac - building_demand.iloc[i]["Sum Electricity [kWh]"] - h2_boiler_energy.p_boiler_el - heat_pump_energies.p_el * system_ops.hp - p_el_fan + ww_storage_balance.p_el_chp - ww_storage_balance.p_el_emc - ww_storage_balance.p_circulation_pumps
            residual_load_electric_initial = residual_load_electric #saving residuals before being stored by EV and Battery
            ...
            p_electricity_grid = - residual_load_electric + electrical_storage_balance.p_electrical_storage
            ...
            energy_system_electrical_temp = [i, ev_df["Dataframe"]["Date"][i],
                                             ...
                                             residual_load_electric, electrical_storage_balance.p_electrical_storage, electrical_storage_balance.e_electrical_storage_t, electrical_storage_balance.soc, p_electricity_grid,ev_df["Dataframe"]["Plug"][i],
            ...
            energy_system_results_electrical.append(energy_system_electrical_temp)
        ...
        column_names_electrical = ["Index","Timestamp","T ambient [°C]","T SRT Air Heatflux [°C]","T Module [°C]","Module Efficiency [-]", "POA Global [W/m2]", "P_el SRT dc [kW]","P_el SRT ac [kW]","Elecricity Demand [kWh el.]", "P_el Heat Pump [kW]", "P_el CHP [kW el.]", "Eta CHP el.", "Pel EMC [kW el.]", "P_el Boiler [kW]", "P_el Fan [kW]", "P el Circulation Pumps [kW]","Residual Load [kW]", "P_el elec. Storage [kW]","E elec. Storage [kWh]","SOC [%]","Grid [kW]","EV Plug Status","EV SOC [%]", "Solar Residuals", "EV P loading [kWh]"]
        energy_system_results_electrical=pd.DataFrame(energy_system_results_electrical, columns=column_names_electrical)

        column_names_thermal = ["Index","Timestamp","Heating Demand Net [kWh th.]","Warm Water Demand [kWh th.]","Total Thermal Energy Demand [kWh th.]","HP COP [-]","q HP [kW th.]","q Boiler [kW th.]","q Reheat Heating Circle [kW th.]","q CHP [kW th.]","q CHP excess [kW th.]","Eta CHP th","H2 cons. CHP [kg H2]", "q in Warm Water Storage [kW th.]","q out Warm Water Storage [kW th.]","Q Warm Water Storage t [kWh th.]","Q Warm Water Storage t-1 [kWh th.]","Cons. Boiler [kg]","OPS"]
        energy_system_results_thermal=pd.DataFrame(energy_system_results_thermal, columns=column_names_thermal)

        column_names_ww_storage = ["Index","Timestamp","Tu [°C]","Tm [°C]","Heat Exchanger SC max [kW]","Heat Exchanger WW max [kW]", "Warm Water Storage [kWh]"]
        warm_water_storage_temperatures=pd.DataFrame(warm_water_storage_temperatures, columns=column_names_ww_storage)
        ...
        electricity_consumption_sum = energy_system_results_electrical["Elecricity Demand [kWh el.]"].copy() + energy_system_results_electrical["P_el Boiler [kW]"].copy() + energy_system_results_electrical["P_el Fan [kW]"].copy()\
                                    + energy_system_results_electrical["Pel EMC [kW el.]"].copy() + energy_system_results_electrical["P el Circulation Pumps [kW]"].copy() + energy_system_results_electrical["P_el Heat Pump [kW]"].copy() + energy_system_results_electrical["EV P loading [kWh]"].copy() # electric consumption in energy system
        thermal_consumption_sum = energy_system_results_thermal["q Boiler [kW th.]"].copy() + energy_system_results_thermal["q CHP [kW th.]"].copy() + energy_system_results_thermal["q HP [kW th.]"].copy() # thermal consumption in energy system
        grid_electricity_consumption_sum = energy_system_results_electrical["Grid [kW]"].copy() # electricity from grid
        gas_consumption_sum = (energy_system_results_thermal["Cons. Boiler [kg]"].copy() + energy_system_results_thermal["H2 cons. CHP [kg H2]"].copy()) * boiler_specs["Ho"] # gas from grid
        autarky_rate_system = autarky_rate_calculation(electricity_consumption_sum, thermal_consumption_sum, grid_electricity_consumption_sum, gas_consumption_sum) # autarky rate calculation regarding electrical and thermal together
        energy_system_results_electrical.insert(loc=9, column='Autarky Rate [-]', value=autarky_rate_system)

        # Heat Pump Share of Grid cover (for seperate billing via Heat Pump Tarif)

        p_el_heat_pump = energy_system_results_electrical["P_el Heat Pump [kW]"].copy()         # Copying Slices of data Frame for grid cover, Heat Pump electrical power, electrical storage output power
        ...
        p_el_heat_pump_share_grid_cover = p_el_heat_pump - p_el_pv - p_el_chp_series + p_el_storage_out     # The share of grid cover for the heat pump is the residual load of the energy storage cover, pv energy share for the HP and CHP generation (due to tandem operation for sys. configuration B), because the heat pump has the first priority when it comes to pv power supply
        ...
        components = [("PVT", building["area_solar_roof_tiles"], specs_eco["Lifetime PVT"]),             # area of solar roof tiles installed is passed, as the PVT Capex are calculated per €/m2
                      ("Fan", fan_heat_pump["p_el"], specs_eco["Lifetime Fan"]),
                      ("Battery", electrical_storage["e_electrical_storage_nom"], specs_eco["Lifetime Battery"]),
                      ("HP", heat_pump["q_nom"], specs_eco["Lifetime HP"]),
                      ("H2 CHP", chp_specs["p_el_nom_chp"], specs_eco["Lifetime H2 CHP"]),
                      ("EMC", energy_system_results_thermal["q CHP excess [kW th.]"].max(), specs_eco["Lifetime EMC"]),
                      ("Water Storage", ww_storage["V_storage"], specs_eco["Lifetime Water Storage"]),
        ...
        if fuel == "NG":
            components.append(("NG Boiler", energy_system_results_thermal["q Boiler [kW th.]"].max(), specs_eco["Lifetime NG Boiler"]))
        else:
            components.append(("H2 Boiler", energy_system_results_thermal["q Boiler [kW th.]"].max(), specs_eco["Lifetime H2 Boiler"]))

        components = pd.DataFrame(data=components, columns=["component", "Capacity", "Lifetime"])
        components, electric_energy_costs, price_dynamics = overall_costs(components, energy_system_results_electrical, energy_system_results_thermal, specs_eco, fuel, specs_renovation, building)

        overall_system_costs = components["CAPEX [€]"].sum() + components["OPEX fix [€]"].sum() + components["OPEX var [€]"].sum() - electric_energy_costs.loc['Bilancy']["Costs/Revenue over runtime [€]"] # electric energy costs have to be subtracted due to negative sign

        overall_system_costs_per_annum = overall_system_costs / n

        components.loc['Sum'] = components.sum() # Last row of components Data Frame should show the sum of costs


        "____Energy_System_-_co2_Emissions___"

        system_co2_emission_timeline, co2_emission_timelines = co2_emission_timeline(energy_system_results_electrical["Grid [kW]"], co2_data, year, n) #Estimate CO2 Emissions over System Lifetime
        overall_system_co2_emissions = system_co2_emission_timeline.sum()
        if fuel == "NG": #add emissions from ng boiler
            overall_system_co2_emissions += energy_system_results_thermal["Cons. Boiler [kg]"].sum() * specs_ecological["ng_emission_factor"] * n
        overall_system_co2_emissions_per_annum = overall_system_co2_emissions / n


        "____Wrapping_Results____"

        res_design = np.array([overall_system_costs_per_annum, overall_system_co2_emissions_per_annum])
        ...
        return_list = [result_designs,energy_system_results_electrical, energy_system_results_thermal, warm_water_storage_temperatures, components, electric_energy_costs, price_dynamics, system_ops, solar_thermal_energy, ww_storage_balance, heat_pump_energies, h2_boiler_energy, electrical_storage_balance, overall_system_co2_emissions, overall_system_costs, system_co2_emission_timeline, co2_emission_timelines]

        return return_list

    return_list_1 = Parallel(n_jobs=optimization_specs["Parallel jobs"])(delayed(threaded_function)(ii, designs,return_list_1) for ii in tqdm(range(len(designs)), desc='Simulating populations..'))
    return_list_2 = []

    for i in range(len(return_list_1)):
        return_list_2.append(return_list_1[i][0])

    if (np.shape(designs) == (1, 9)) or (np.shape(designs) == (1, 10)): # Function Output in Non-Optimization Mode with CHp
        print("\nReturn Non-Optimization Mode results")
        return return_list_1[0]     #returns only first row with all results of first simulation
    else: # Function Output in Optimization Mode
        return return_list_2        #returns only list of all results_desigs (costs and co2 emissions) in the generation being simulated
...
if output_results_as_table == True:
    if optimization_mode == True:
        ...
        if combined_heat_and_power == True:
            ...
            pareto_front_df = pd.DataFrame(optimization_results.F, columns=["Overall Costs [€/a]","CO2äq [kg/a]"])
            optimal_solutions_df = pd.DataFrame(optimization_results.X, columns=["PVT East [kW el]","PVT West [kW el]", "BESS [kWh el]", "Storage [L]", "ASHP [kW th]", "t biv [°C]", "CHP [kWel]", "Type of Boiler Fuel", "EV-EMS Case","Renovation Case"])

        else:

            pareto_front_df = pd.DataFrame(optimization_results.F, columns=["Overall Costs [€/a]","CO2äq [kg/a]"])
            optimal_solutions_df = pd.DataFrame(optimization_results.X, columns=["PVT East [kW el]","PVT West [kW el]", "BESS [kWh el]", "Storage [L]", "ASHP [kW th]", "t biv [°C]", "Type of Boiler Fuel", "EV-EMS Case","Renovation Case"])

        hyper_volume_spacing_df = pd.DataFrame({"HV": [hyper_volume_indicator], "S": [spacing]})
        
        try: 
            #create the results folder first if it does not exist
            folder_name = os.path.join(path+"\Output\Optimizations", dt.date.today().strftime('%Y-%m-%d'))
            os.makedirs(folder_name)
            
            pareto_front_df.to_excel(os.path.join("Output\Optimizations\{}", r'ParetoFront_Sys_{}_Algorithm_{}_Gen_{}_Pop_{}_Price_{}_Date_{}.xlsx').format(dt.date.today(),sys_config,optimization_specs["Algorithm"],optimization_specs["Generations"],optimization_specs["Population Size"],specs_eco["h2 price"],dt.datetime.now().strftime("%Y-%m-%d")))
            optimal_solutions_df.to_excel(os.path.join("Output\Optimizations\{}", r'ParetoOptimalSolutions_Sys_{}_Algorithm_{}_Gen_{}_Pop_{}_Price_{}_Date_{}.xlsx').format(dt.date.today(),sys_config,optimization_specs["Algorithm"],optimization_specs["Generations"],optimization_specs["Population Size"],specs_eco["h2 price"],dt.datetime.now().strftime("%Y-%m-%d")))
            hyper_volume_spacing_df.to_excel(os.path.join("Output\Optimizations\{}", r'HV_S_Sys_{}_Algorithm_{}_Gen_{}_Pop_{}_Price_{}_Date_{}.xlsx').format(dt.date.today(),sys_config,optimization_specs["Algorithm"],optimization_specs["Generations"],optimization_specs["Population Size"],specs_eco["h2 price"],dt.datetime.now().strftime("%Y-%m-%d")))
        except:
            pareto_front_df.to_excel(os.path.join("Output\Optimizations\{}", r'ParetoFront_Sys_{}_Algorithm_{}_Gen_{}_Pop_{}_Price_{}_Date_{}.xlsx').format(dt.date.today(),sys_config,optimization_specs["Algorithm"],optimization_specs["Generations"],optimization_specs["Population Size"],specs_eco["h2 price"],dt.datetime.now().strftime("%Y-%m-%d")))
            optimal_solutions_df.to_excel(os.path.join("Output\Optimizations\{}", r'ParetoOptimalSolutions_Sys_{}_Algorithm_{}_Gen_{}_Pop_{}_Price_{}_Date_{}.xlsx').format(dt.date.today(),sys_config,optimization_specs["Algorithm"],optimization_specs["Generations"],optimization_specs["Population Size"],specs_eco["h2 price"],dt.datetime.now().strftime("%Y-%m-%d")))
            hyper_volume_spacing_df.to_excel(os.path.join("Output\Optimizations\{}", r'HV_S_Sys_{}_Algorithm_{}_Gen_{}_Pop_{}_Price_{}_Date_{}.xlsx').format(dt.date.today(),sys_config,optimization_specs["Algorithm"],optimization_specs["Generations"],optimization_specs["Population Size"],specs_eco["h2 price"],dt.datetime.now().strftime("%Y-%m-%d")))
...
if plot_results ==True:
    if optimization_mode == True:
        try: 
            plot_Pareto_Fronts(optimization_mode= optimization_mode, opt_results= optimization_results.F.T, optimal_solutions_df= optimal_solutions_df, sys_config= sys_config,optimization_specs=optimization_specs)
        
        except:
            """ Use this part for plotting imported excel results """
            #importing pareto front
            pareto_front_import = pd.read_excel(r"C:\Users\Fares\Documents\GitHub\Multi-Objektive-Optimization\multi-objektive-optimization\Output\Optimizations\2024-10-23\ParetoFront_Sys_A_Algorithm_NSGA2_Gen_60_Pop_50_Price_9.28_Date_2024-10-23.xlsx")
            #importing optimal solutions 
            optimal_solutions_import = pd.read_excel(r"C:\Users\Fares\Documents\GitHub\Multi-Objektive-Optimization\multi-objektive-optimization\Output\Optimizations\2024-10-23\ParetoOptimalSolutions_Sys_A_Algorithm_NSGA2_Gen_60_Pop_50_Price_9.28_Date_2024-10-23.xlsx")
            ...
            plot_Pareto_Fronts(optimization_mode= optimization_mode, opt_results= opt_ref , optimal_solutions_df= optimal_solutions_import, sys_config= "A",optimization_specs=optimization_specs)
...
if plot_results ==True:
    if optimization_mode == False: # plot normal Timerseries calculation results
        ...
        input_data_year = energy_system_results_electrical.Timestamp.dt.year[0] # Get year of input data
        n_ticks = np.zeros(12)  # empty array for 12 ticks for each month of the year
    
        for i in range(len(n_ticks)-1):
            num_days_month = calendar.monthrange(input_data_year, i+1)[1] # Get number of days for each month of year of input data
            n_ticks[i+1] = n_ticks[i] + num_days_month*24    # Add number of hours for each month to get xticks position in hours
        ...
        if combined_heat_and_power == False:    # plot results without CHP operation
            ...
            energy_system_results_electrical.drop(columns=['Index',
                                                           ...
                                                           "Solar Residuals"]
                                                            ...
                                                            ax=axes,
                                                            xlabel = '{} - {}'.format(energy_system_results_electrical.Timestamp.dt.strftime('%d.%m.%Y')[0], energy_system_results_electrical.Timestamp.dt.strftime('%d.%m.%Y')[len(energy_system_results_electrical)-1]),
            ...
            plt.savefig(path + '\\Output\\Timeseries Results\\Timeseries Energy System Sys. A Electrical {}.png'.format(dt.datetime.now().strftime("%Y-%m-%d %H-%M-%S")), dpi=300, format='png', bbox_inches='tight')
            ...
            plt.savefig(path + '\\Output\\Timeseries Results\\Timeseries Energy System Sys. A Thermal {}.png'.format(dt.datetime.now().strftime("%Y-%m-%d %H-%M-%S")), dpi=300, format='png', bbox_inches='tight')
            ...
            plt.savefig(path + '\\Output\\Timeseries Results\\Timeseries Energy System Sys. A EV {}.png'.format(dt.datetime.now().strftime("%Y-%m-%d %H-%M-%S")), dpi=300, format='png', bbox_inches='tight')
        ...
        # Energy System - Results electrical
            ...
            plt.savefig(path + '\\Output\\Timeseries Results\\Timeseries Energy System Sys. B Electrical {}.png'.format(dt.datetime.now().strftime("%Y-%m-%d %H-%M-%S")), dpi=300, format='png', bbox_inches='tight')
            ...
            plt.savefig(path + '\\Output\\Timeseries Results\\Timeseries Energy System Sys. B Thermal {}.png'.format(dt.datetime.now().strftime("%Y-%m-%d %H-%M-%S")), dpi=300, format='png', bbox_inches='tight')
            ...
            plt.savefig(path + '\\Output\\Timeseries Results\\Timeseries Energy System Sys. B EV {}.png'.format(dt.datetime.now().strftime("%Y-%m-%d %H-%M-%S")), dpi=300, format='png', bbox_inches='tight')
...
if Sensitivity_analysis == True:
    Sensitivity_electricity.to_excel(os.path.join("Output\Optimizations\{}",r"Sensitivity_Analysis_of_Electricity_Prices_{}_Algorithm_{}_Gen_{}_Pop_{}_Price_{}_Date_{}.xlsx").format(dt.date.today(),sys_config,optimization_specs["Algorithm"],optimization_specs["Generations"],optimization_specs["Population Size"],specs_eco["h2 price"],dt.datetime.now().strftime("%Y-%m-%d")))
    Sensitivity_h2.to_excel(os.path.join("Output\Optimizations\{}",r"Sensitivity_Analysis_of_h2_Prices_{}_Algorithm_{}_Gen_{}_Pop_{}_Price_{}_Date_{}.xlsx").format(dt.date.today(),sys_config,optimization_specs["Algorithm"],optimization_specs["Generations"],optimization_specs["Population Size"],specs_eco["h2 price"],dt.datetime.now().strftime("%Y-%m-%d")))
    Sensitivity_ng.to_excel(os.path.join("Output\Optimizations\{}",r"Sensitivity_Analysis_of_ng_Prices_{}_Algorithm_{}_Gen_{}_Pop_{}_Price_{}_Date_{}.xlsx").format(dt.date.today(),sys_config,optimization_specs["Algorithm"],optimization_specs["Generations"],optimization_specs["Population Size"],specs_eco["h2 price"],dt.datetime.now().strftime("%Y-%m-%d")))
...
if plot_results == True:
    if Sensitivity_analysis == True:
        ...
        try:
            ...
            plotting_Sensitivity(opt_ref,Sensitivity_electricity, "electricity")    
            ...
            Sensitivity_electricity = pd.read_excel(r"C:\Users\Fares\Documents\GitHub\Multi-Objektive-Optimization\multi-objektive-optimization\Output\Optimizations\2024-10-23\Sensitivity_Analysis_of_Electricity_Prices_A_Algorithm_NSGA2_Gen_60_Pop_50_Price_9.28_Date_2024-10-23.xlsx")
            Sensitivity_h2 =pd.read_excel(r"C:\Users\Fares\Documents\GitHub\Multi-Objektive-Optimization\multi-objektive-optimization\Output\Optimizations\2024-10-23\Sensitivity_Analysis_of_h2_Prices_A_Algorithm_NSGA2_Gen_60_Pop_50_Price_9.28_Date_2024-10-23.xlsx")
            Sensitivity_ng =pd.read_excel(r"C:\Users\Fares\Documents\GitHub\Multi-Objektive-Optimization\multi-objektive-optimization\Output\Optimizations\2024-10-23\Sensitivity_Analysis_of_ng_Prices_A_Algorithm_NSGA2_Gen_60_Pop_50_Price_9.28_Date_2024-10-23.xlsx")
            ...
            #creating plots
            plotting_Sensitivity(opt_ref,Sensitivity_results= Sensitivity_electricity, Sensitivity_test= "electricity")
            plotting_Sensitivity(opt_ref,Sensitivity_h2, "h2")
            plotting_Sensitivity(opt_ref,Sensitivity_ng, "ng")
...
if plot_results == True:
    try:  
        plot_PCP(ResultPath                 = path + "/Output\Optimizations/Illustrations/{}".format(dt.date.today()), 
             ParetoFrontPath_Opt_Sol    = path + "/Output/Optimizations/{}/ParetoOptimalSolutions_Sys_{}_Algorithm_{}_Gen_{}_Pop_{}_Price_{}_Date_{}.xlsx".format(dt.date.today(),sys_config,optimization_specs["Algorithm"],optimization_specs["Generations"],optimization_specs["Population Size"],specs_eco["h2 price"],dt.datetime.now().strftime("%Y-%m-%d")), 
